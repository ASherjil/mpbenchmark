...
void InitializeArray(){
	FILE * file;
	file=fopen( "..//IOFiles//InputFile.txt", "r" );
	double a,b,c,d;
	LineCount=0;
	while (!feof(file))
   	{
    ...
    inputArray = (double**)malloc(sizeof(double*)*LineCount); 
    outputArray = (double**)malloc(sizeof(double*)*LineCount); 
    int i;
    for (i=0; i<LineCount; i++){
        inputArray[i] = (double*)malloc(sizeof(double)*4);
        outputArray[i] = (double*)malloc(sizeof(double)*18);
 	}
   	...
   	{
   		fscanf(file,"%lf%lf%lf%lf",&a,&b,&c,&d);
   		inputArray[index][0]=a;
   		inputArray[index][1]=b;
   		inputArray[index][2]=c;
   		inputArray[index][3]=d;
   		index++;
   	}
   	fclose( file );
   	//for(i=0;i<LineCount;i++){
